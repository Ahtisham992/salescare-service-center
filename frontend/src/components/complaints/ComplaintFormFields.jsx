  ...
  const handleCreateCustomer = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setFormData(prev => ({ ...prev, customer_id: result.data.customer_id }));
        ...
        setNewCustomerData({ name: '', phone: '', address: '', cnic: '' });
    ...
    <div className="grid grid-cols-2 gap-4">
      ...
      <div className="col-span-2 md:col-span-1">
        ...
        {isEditing ? (
          ...
          <select value={formData.customer_id} className="form-input bg-gray-50" disabled>
            <option value={formData.customer_id}>
              {customers.find(c => c.customer_id === formData.customer_id)?.name || 'Selected Customer'}
            </option>
          </select>
        ) : showNewCustomerForm ? (
          ...
          <div className="bg-white p-4 rounded-lg border-2 border-primary-100 shadow-sm animate-in fade-in zoom-in duration-200">
            <div className="flex justify-between items-center mb-3 border-b pb-2">
              <span className="text-sm font-bold text-gray-800 flex items-center">
                <Plus className="w-4 h-4 mr-1 text-primary-600" /> Add New Customer
              </span>
              <button 
                  ...
                  filteredCustomers.map((customer) => (
                    <button
                      ...
                      className={`w-full text-left px-4 py-3 hover:bg-gray-50 border-b border-gray-100 last:border-0 transition-colors ${
                        formData.customer_id === customer.customer_id ? 'bg-primary-50' : ''
                      }`}
                    >
                      <div className="font-medium text-gray-900 flex justify-between">
                        {customer.name}
                        {formData.customer_id === customer.customer_id && <span className="text-primary-600 text-xs font-bold">Selected</span>}
                      </div>
                      <div className="text-sm text-gray-500 mt-0.5">
                        {customer.phone}
                        {customer.cnic && <span className="text-gray-400"> • {customer.cnic}</span>}
                      </div>
                    </button>
                  ))
                )}
      ...
      <div className="col-span-2 md:col-span-1">
        ...
        {isEditing && <p className="text-xs text-gray-500 mt-1">🔒 Locked after creation</p>}
      </div>
